...
let append_field s f =
  foldi
    (fun a info ->
      let type_name =
        match f.Cil.ftype with
        | Cil.TComp (cinfo, _) -> cinfo.Cil.cname
        | _ -> "-"
      in
      if f.Cil.fname = "archive" && type_name = "archive" then
        PowLoc.add (Loc.of_allocsite a)
      else if PowStruct.mem f.Cil.fcomp.Cil.cname info.ArrInfo.structure then
