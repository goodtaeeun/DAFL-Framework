...
let remove_unreachable_funcs global =
  let pids_all = PowProc.of_list (InterCfg.pidsof global.icfg) in
  let entry = InterCfg.global_proc in
  let reachable = CallGraph.trans_callees entry global.callgraph in
  let unreachable =
  ...
  let global =
    ...
    else remove_funcs unreachable global
  in
