...
(* Return top 'n' functions that have the most callers *)
let filter_by_caller_until n g =
  let folder v acc = (v, List.length (G.pred g.graph v)) :: acc in
  G.fold_vertex folder g.graph []
  |> List.sort (fun x y -> Stdlib.compare (snd x) (snd y))
  |> List.rev
  (* DEBUG *)
  |> (fun entries ->
       List.iter (fun (v, c) -> Logging.info "%s: %d callers\n" v c) entries;
       entries)
  |> List.map fst
  |> add_until n g PowProc.empty
  |> PowProc.elements
