...
let get_inline_targets global =
  if !Options.pcs_ratio > 0 then (
    let n = float (CallGraph.size global.callgraph) in
    let ratio = float !Options.pcs_ratio /. 100.0 in
    let thres = Float.round (n *. ratio) |> Float.to_int in
    L.info "Select up to %d functions to inline\n" thres;
    CallGraph.filter_by_caller_until thres global.callgraph)
  else
    let regexps =
...
let inline global =
  ...
  let to_inline = get_inline_targets global in
