...
let transform global =
  ...
  let global =
    if (not !Options.il) && (loop_transformed || inlined) then
      (* NOTE: CFG must be re-computed if something changed *)
      Frontend.makeCFGinfo global.file
      |> StepManager.stepf true "Graph construction after transform" Global.init
      |> StepManager.stepf true "Pre-processing after transform" PreProcess.run
    else global
  in
...
let init_analysis file =
  if !Options.marshal_in then marshal_in file
    ...
    |> build_line_to_func_map
    |> opt_exit !Options.dump_all DugSlicer.dump_funcs
    |> StepManager.stepf true "Pre-processing" PreProcess.run
    |> opt_exit !Options.dump_refer DugSlicer.dump_funcs
    |> transform
    |> StepManager.stepf true "Pre-analysis" PreAnalysis.run
    |> opt_exit !Options.dump_reach DugSlicer.dump_funcs
...
let print_cfg global =
  let oc = open_out (Filename.concat !Options.outdir "node.json") in
