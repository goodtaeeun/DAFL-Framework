...
let dump_refer = ref false

let dump_reach = ref false
...
let ctx_insen_slice = ref false

let unfiltered_slice = ref false
...
let opts =
  [
    ...
    ("-dump_all", Arg.Set dump_all, "Dump all the functions");
    ("-dump_refer", Arg.Set dump_refer, "Dump referred functions");
    ("-dump_reach", Arg.Set dump_reach, "Dump reachable functions");
    ( "-slice",
      Arg.String (fun s -> add_slice_target s),
      "Slice w.r.t a given target" );
    ("-full_slice", Arg.Set full_slice, "Perform full (not thin) slicing");
    ( "-ctx_insen_slice",
      Arg.Set ctx_insen_slice,
      "Perform context-insensitive slicing" );
    ( "-unfiltered_slice",
      Arg.Set unfiltered_slice,
      "Perform slicing without reachability-based location filtering" );
    ...
    ( "-pcs",
      Arg.Set_int pcs_ratio,
      "Partial context-sensitivity [0-100] (0: context-insensitive, 100: fully \
       context sensitive). default=0" );
    ( "-pfs",
      Arg.Set_int pfs,
    ...
    ("-unwrap", Arg.Set unwrap, "Unwrap functions");
  ]
